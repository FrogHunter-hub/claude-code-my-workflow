...
% \note{Present examples of transcripts with high demand-pull cause shares vs.\ high technology-push cause shares for the same technology.  The reader should be able to verify that the taxonomy labels match the underlying text.  This is the analog of the top-scoring transcript tables in Hassan et al.\ (2019, Online Appendix Table XX) and Sautner et al.\ (2023, Table II).}
...
For fracking, the tech-push excerpt describes ``technology advances in horizontal drilling and hydraulic fracturing techniques,'' pure engineering innovation, while the demand-pull excerpt discusses ``sustained demand for hydraulic fracturing applications in the energy sector.''
...
In each case, the same technology is discussed through qualitatively different causal lenses.

Coverage is broad: all 29 technologies have a clear tech-push excerpt, and 27 of 29 have a demand-pull excerpt (the exceptions, computer vision and drug conjugates, lack cause-side spans classified as Market Demand).
...
The taxonomy is constructed by the LLM without knowledge of each technology's economic context; the question is whether the resulting category shares align with the adoption narratives that domain knowledge would predict.

On the cause side, the cross-technology variation is consistent with standard accounts of each technology's adoption.
Technologies that economists would characterize as supply-driven are also framed in supply-push terms by managers: drug conjugates, virtual reality, computer vision, and machine learning are dominated by this category, reflecting the central role of R\&D breakthroughs and platform capabilities in their diffusion.
At the other end of the spectrum, technologies typically associated with consumer adoption, such as mobile payments, social networking, wireless charging, are framed predominantly in demand-pull terms by managers.
Solar power and hybrid electric vehicles stand out for their substantial Regulatory and Policy Drivers shares, consistent with the view that managers perceive government mandates and subsidies as central to clean-energy adoption.
Fracking, whose diffusion is commonly attributed to both drilling innovation and commodity economics, exhibits a characteristic mix of Technology Innovation and Cost and Economic Viability in managerial discourse.

Figure~\ref{fig:techpush-demandpull} isolates the two dominant cause categories---Technology Innovation and Advancement (technology-push) and Market Demand and Consumer Behavior (demand-pull)---in a scatter plot that places each technology according to its span-weighted shares on these two dimensions.
The resulting map echoes the classic technology-push versus demand-pull debate in the economics of innovation.
Drug conjugates, machine learning, and computer vision cluster in the lower right: their adoption narratives are overwhelmingly supply-driven, with technology-push shares exceeding 55\% and near-zero demand-pull shares.
Cloud computing, social networking, and smart devices lie above the equal-share diagonal, with demand-pull shares of 39--43\%, indicating that managers frame these technologies primarily in terms of customer and market forces.
Twenty of the 26 included technologies fall below the diagonal, consistent with the observation from Table~\ref{tab:summary-stats} that Technology Innovation and Advancement is the single most prevalent cause category in the sample.
Circle area, proportional to total cause span count, shows that the most heavily discussed technologies---cloud computing (28,071 spans), solar power (9,396), and smart devices (7,971)---span a wide range of the technology-push/demand-pull spectrum, confirming that the variation is not an artifact of sample size.

The effect side shows parallel patterns (Panel~B).
Consumer-facing technologies such as mobile payments and social networking are framed pre-dominantly in terms of Market Expansion and Adoption effects, consistent with growth-oriented expectations that managers associate with network goods.
R\&D-intensive technologies such as drug conjugates, computer vision, and solar power show large Product and Service Innovation shares in managerial narratives.

Technologies that managers describe as deployed for operational improvement, such as fracking and RFID tags, have above-average Cost Reduction and Efficiency shares.
...
% \note{Show cause and effect compositions vary across technologies in expected ways: solar power = regulation-heavy causes, ML/AI = technology innovation-heavy, mobile payments = demand-heavy (Figure~I, bar chart or heat map).  The question to answer: does the taxonomy recover the known drivers of each technology without being told what to look for?  This parallels the sector-level variation checks in Hassan et al.\ (2019, Figure III) and the emissions-correlation checks in Sautner et al.\ (2023, Section III.B.3).}

\subsection*{III.C. Time-Series Patterns}

% \note{Document aggregate time-series patterns.  Platform technologies (cloud, ML/AI): slow, sustained ramp.  Feature technologies (touchscreen, RFID): spike and decline.  Policy-driven technologies (hybrid EV, solar): stop-go waves correlated with known policy events (Paris Agreement, IRA, etc.).  This validates the time-series content of the measures.  Figure~II should present 2--3 examples, with event markers.}
...
Panel~A presents two platform technologies, cloud computing and machine learning/AI, that exhibit a sustained ramp in discussion prevalence. Cloud computing enters the earnings-call vocabulary around 2008 and climbs steadily through 2022, consistent with the widely documented enterprise migration to cloud infrastructure. Machine learning/AI follows a similar but later trajectory, accelerating after 2016 as deep-learning applications reach production scale, with a further sharp increase in late 2022 coinciding with the release of large language models. Neither technology shows the transient spike of a short-lived trend; discussions grow steadily over time.

Panel~B illustrates the opposite pattern for two feature technologies. RFID tags spike in the mid-2000s, peaking shortly after Walmart's 2003 supplier mandate drew industry attention, then decline as supply-chain tagging matures into routine practice.
Touch screens follow a similar arc on a smaller scale, with discussion rising after the 2007 iPhone launch and fading as capacitive displays became ubiquitous.
...
Solar power discussion surges after the American Recovery and Reinvestment Act (2009), contracts as stimulus funding expires, revives around the Paris Agreement and the solar investment tax credit extension (2015), dips following the imposition of solar tariffs (2018), and rebounds with the Inflation Reduction Act (2022). Hybrid electric vehicles trace a more subdued but temporally similar path.
The correspondence between policy milestones and discussion prevalence provides external validation that the measures respond to real-world adoption incentives rather than noise.

Taken together, the three panels show that the discussion-prevalence measures track qualitatively distinct diffusion patterns across technology types. The within-quarter firm-level variation that drives the analyses in Sections~IV--VI sits on top of these aggregate patterns.
...
For each firm--technology--quarter observation $(i,k,t)$, we construct a leave-one-out \textit{cross-technology peer share}: the firm's average share in category $c$ across all its other technologies $j \neq k$, pooled over all quarters. ``Peer'' here refers to peer technologies within the same firm's portfolio, not to peer firms. We suppress the cause/effect superscript where the side is clear from context.  The cross-technology peer share for firm $i$, technology $k$, category $c$ is
\begin{equation}
\label{eq:peer_share}
...
Column~IV restricts the sample to the first quarter in which each firm discusses a given technology (1,925 observations before stacking).  This is the most direct test of belief portability: when a firm encounters a new technology for the first time, does its initial causal frame resemble its existing portfolio?  The coefficient is 0.049 ($p < 0.05$).  The initial frame a manager brings to a new technology is shaped by the frames already in place for the firm's existing technologies.

Panel~B repeats the analysis for effect shares.  The spillover coefficients are larger and more precisely estimated throughout: 0.099 in the baseline (column~I), 0.103 with the most demanding fixed effects (column~III), and 0.087 at first appearance (column~IV).  All estimates are significant at the 1\% level.  The stronger effect-side spillover is consistent with the variance decomposition, which showed a larger firm-level component for effect shares (67--74\%) than for cause shares (60--67\%).  The portability of effect-side frames is particularly pronounced, exceeding the cause-side portability across all specifications. .
...
\note{the data pattern doesn't support a 'transition story', as the cause-effect ratio is relatively stable. For more details can refer to Methodology/2026-02-13_dynamics-methods-summary.tex}
