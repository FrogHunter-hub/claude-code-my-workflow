...
Table~\ref{tab:spillover_robust} presents four robustness checks.  First, restricting the sample to firms that discuss at least three technologies---thereby removing the 562 two-technology firms that contribute noisier peer shares---strengthens the result: the cause-side coefficient roughly doubles to 0.147 ($p < 0.01$) and the effect-side coefficient increases to 0.160 ($p < 0.01$).  Second, weighting observations by the number of underlying snippets, which downweights observations where shares are measured less precisely, yields estimates close to the baseline (0.061 for causes, 0.105 for effects).

Third, sequentially dropping each of the five categories from the stacked regression produces a narrow range of coefficients (0.032 to 0.075 for causes, 0.074 to 0.114 for effects), confirming that no single category drives the result mechanically through the constraint that shares sum to one.  Fourth, interacting the peer share with the cosine similarity between the focal technology's and the peer technologies' aggregate cause-share vectors yields small and insignificant interaction terms on both sides; the implied marginal effects at the 25th and 75th percentiles of similarity are nearly identical (0.066 and 0.063 for causes, 0.095 and 0.104 for effects), suggesting that spillover operates broadly across the firm's technology portfolio rather than concentrating among similar technologies.
